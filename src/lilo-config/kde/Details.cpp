...
#include <ui.h>
#include <kvbox.h>
Details::Details(liloimage *lilo, QWidget *parent, const char *name, Qt::WFlags /* f */)
 : KDialog(parent)
{
	l=lilo;
	...
	KHBox *vgab=new KHBox(page);
	QLabel *vlbl=new QLabel(_("&Graphics mode on text console:"), vgab);
	vga=new QComboBox(false, vgab);
	vlbl->setBuddy(vga);
	vgab->setWhatsThis( _("You can select the graphics mode for this kernel here.<br>If you intend to use a VGA graphics mode, you must compile the kernel with support for framebuffer devices. The <i>ask</i> setting brings up a prompt at boot time."));
	...
	KHBox *opts=new KHBox(page);
	lock=new QCheckBox(_("&Record boot command lines for defaults"), opts);
	lock->setWhatsThis( "<qt>" + _("Checking this box enables automatic recording of boot command lines as the default for the following bootups. This way, lilo \"locks\" on a choice until it is manually overridden.<br>This sets the <b>lock</b> option in lilo.conf"));
