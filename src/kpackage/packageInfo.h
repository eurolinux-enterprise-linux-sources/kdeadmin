/*
** Copyright (C) 1999,20007 Toivo Pedaste <toivo@ucs.uwa.edu.au>
**
//
...
public:
   ...
   */
  QString getInfo(const QString &property);
...
// Delete a property
  ...
  void updateItem();
  // update package info in package tree
  
  void pkgFileIns(const QString &fileName);
  // Insert a package from a file into package tree

  bool pkgInsert(QList<packageInfo *> &pki,  bool installed,
	      bool infoPackage = false);
  // insert packgeInfo either installed or not installed

  void pkgUpdate(packageInfo *pc, packageInfo *pi, bool installed, bool hidden);
  // 
  ...
  short packType;
  // Whether DEB, RPM or SLACK package
  
  QStringList *depends ;
  QStringList *files;
   // Dependency and files information
