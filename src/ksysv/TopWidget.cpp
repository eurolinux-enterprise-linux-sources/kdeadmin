...
KSVTopLevel::KSVTopLevel()
  : KMainWindow(0, 0L, Qt::WStyle_ContextHelp|Qt::WDestructiveClose),
...
void KSVTopLevel::initActions ()
{
  ...
  // setup File menu
  mFileRevert = KStandardAction::revert (this, SLOT (slotClearChanges()), coll);
  mFileRevert->setText (i18n("Re&vert Configuration"));

  mFileLoad = KStandardAction::open (this, SLOT (load()), coll);
  mFileLoad->setText (i18n ("&Open..."));

  mFileSave = KStandardAction::save(this, SLOT(slotAcceptChanges()), coll);
  mFileSave->setText (i18n("&Save Configuration"));

  mFileSaveAs = KStandardAction::saveAs (this, SLOT (saveAs ()), coll);

  mFileSaveLog = KStandardAction::save (this, SLOT(slotSaveLog()), coll, "ksysv_save_log");
  mFileSaveLog->setText (i18n("Save &Log..."));
  mFileSaveLog->setShortcut (QKeySequence(Qt::Key_L+Qt::CTRL));
  mFileSaveLog->setEnabled (false);

  // disabled due to complexity
  //  mFilePrint = KStandardAction::print (this, SLOT (print()), coll);

  mFilePrintLog = KStandardAction::print(this, SLOT(printLog()), coll, "ksysv_print_log");
  mFilePrintLog->setText( i18n("&Print Log..."));
  mFilePrintLog->setEnabled (false);

  mFileQuit = KStandardAction::quit(this, SLOT(close()), coll);

  // setup Edit menu
  mEditUndo = KStandardAction::undo(this, SLOT(editUndo()), coll);
  mEditUndo->setEnabled (false);
  mEditRedo = KStandardAction::redo(this, SLOT(editRedo()), coll);
  mEditUndo->setEnabled (false);
  mEditCut = KStandardAction::cut(this, SLOT(editCut()), coll);
  mEditCopy = KStandardAction::copy(this, SLOT(editCopy()), coll);
  mEditPaste = KStandardAction::paste(this, SLOT(editPaste()), coll);
  mPasteAppend = KStandardAction::paste (this, SLOT (pasteAppend()),
  ...
  KStandardAction::keyBindings (this, SLOT(configureKeys()), coll);
  KStandardAction::configureToolbars (this, SLOT(configureToolbars()), coll);
  KStandardAction::saveOptions(this, SLOT(saveOptions()), coll);
  KStandardAction::preferences(this, SLOT(slotShowConfig()), coll);
...
void KSVTopLevel::initStatusBar()
{
  ...
  Q3HBox* visBox = new Q3HBox (status, "visBox");
  Q3ButtonGroup* group = new Q3ButtonGroup (this, "visButtonGroup");
  group->hide();
  connect (group, SIGNAL (clicked (int)), this, SLOT (toggleRunlevel (int)));

  visBox->setWhatsThis( i18n ("<p>Click on the checkboxes to <strong>show</strong> or "\
  ...
  visBox->setToolTip( i18n ("Show only the selected runlevels"));
  ...
  Q3HBox* authIconBox = new Q3HBox (status, "AuthIconBox");
  ...
  authIconBox->setWhatsThis( i18n ("<p>If the lock is closed <img src=\"user|ksysv_locked\"/>, "\
                                      ...
                                      "or ask your sysadmin to install %2 <em>suid</em> or " \
  ...
  status->setItemAlignment (Status::Changed, Qt::AlignLeft|Qt::AlignVCenter);
}
  ...
  if (list && list->currentItem())
	{
	  ...
	  QApplication::clipboard()->setData (mime);
...
void KSVTopLevel::setChanged (bool val)
{
  ...
  // update statusbar
  statusBar()->changeItem(val ? i18n(" Changed") : QString(), Status::Changed);
...
//   static QPrinter prt;
//   prt.setDocName(KGlobal::mainComponent().aboutData()->programName() + " Configuration");
//   prt.setCreator(KGlobal::mainComponent().aboutData()->programName());
...
//           p.drawText (10, y, i18n ("Runlevel %1",i));
//           y += fm.lineSpacing();
...
void KSVTopLevel::printLog()
{
  ...
  prt->setDocName(KGlobal::mainComponent().aboutData()->programName() + " Log File");
  prt->setCreator(KGlobal::mainComponent().aboutData()->programName());

//  if (prt->setup(this, i18n("Print Log File")))
    QPrintDialog *printDialog = KdePrint::createPrintDialog(prt, parent);
    if (printDialog->exec())
    {
      KBusyManager::self()->setBusy (true);

      QPainter p (prt);
      Q3PaintDeviceMetrics pm (prt);
      ...
      {
        Q3SimpleRichText rheading (i18n("<h1>KDE Sys-V Init Editor Log</h1>"),
                                  QFont("times"), QString(), ksv::styleSheet(),
        ...
        Q3SimpleRichText rdate (i18n("<h3>Printed on %1</h3><br/><br/>", KGlobal::locale()->formatDateTime(QDateTime::currentDateTime())),
                               QFont("times"), QString(), ksv::styleSheet(),
                               Q3MimeSourceFactory::defaultFactory());
      ...
      while (line != lines.end())
        {
          while (y < height)
            {
              Q3SimpleRichText rich (*line, QFont("times"), QString(), ksv::styleSheet(),
                                    Q3MimeSourceFactory::defaultFactory());
...
void KSVTopLevel::slotSaveLog()
{
  ...
  file.open( QIODevice::WriteOnly | IO_Raw );
  QTextStream s(&file);
...
{
  setCaption(KNetwork::KResolver::localHostName(), changed);
}
...
void KSVTopLevel::load ()
{
  ...
  file.open (QIODevice::ReadOnly | IO_Raw);
  QDataStream s (&file);
  Q3ValueList<KSVData>* startLists = new Q3ValueList<KSVData>[ksv::runlevelNumber];
  Q3ValueList<KSVData>* stopLists = new Q3ValueList<KSVData>[ksv::runlevelNumber];
  ...
  if (ksv::IO::loadSavedConfiguration (s, startLists, stopLists))
    ...
    {
      kDebug (3000) << "ERROR loading file";
    }
