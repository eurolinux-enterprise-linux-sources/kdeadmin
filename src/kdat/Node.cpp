...
bool TapeNode::validate()
{
    ...
    Q3PtrListIterator<Archive> i( _tape->getChildren() );
    ArchiveNode* n;
    for ( ; i.current(); ++i ) {
...
void TapeNode::expanding( bool expand )
{
    ...
    Q3PtrListIterator<Archive> f( _tape->getChildren() );
    for ( ; f.current(); ++f ) {
...
void ArchiveNode::expanding( bool expand )
{
    ...
    Q3PtrListIterator<File> f( _archive->getChildren() );
    for ( ; f.current(); ++f ) {
          ...
          _file( file )
{
    ...
    int idx1 = _file->getName().lastIndexOf( '/', len - 2 );
    if ( idx1 < 0 ) {
...
void TapeDirectoryNode::expanding( bool expand )
{
    ...
    Q3PtrListIterator<File> f( _file->getChildren() );
    for ( ; f.current(); ++f ) {
        if ( f.current()->getName()[f.current()->getName().length()-1] == '/' ) {
            insertNode( new TapeDirectoryNode( f.current() ) );
        } else {
...
{
    int len  = _file->getName().length();
    int idx1 = _file->getName().lastIndexOf( '/', len - 1 );
    if ( idx1 < 0 ) {
                                ...
                                const QColorGroup& cg, bool highlighted ) const
{
    ...
    if (highlighted) {
        QStyle *style = owner->style(); 
	Qt::GUIStyle sty;
	// FIXME
	//.styleHint(QStyle::SH_GUIStyle)); // Qt3 doesn't make this easy ;)
	style->styleHint ( QStyle::SH_GUIStyle );
...
const Q3PtrList<Range>& MountedArchiveNode::getRanges()
{
...
const Q3PtrList<Range>& MountedTapeDirectoryNode::getRanges()
{
...
const Q3PtrList<Range>& MountedTapeFileNode::getRanges()
{
    return _file->getRanges();
}
...
void RootNode::expanding( bool expand )
{
    ...
    if ( dir.exists() ) {
        ...
        QList<QFileInfo> list = dir.entryInfoList( QDir::Hidden | QDir::Files | QDir::Dirs, QDir::Name | QDir::DirsFirst );
        if ( ! list.isEmpty() ) {
	    ...
	    while ( it.hasNext() ) {
                ...
                if ( ( finfo.fileName() != "." ) && ( finfo.fileName() != ".." ) ) {
                    if ( finfo.isDir() ) {
                        appendChild( new DirectoryNode( finfo.fileName(), _state == SelSome ? SelNone : _state ) );
                    } else {
                        appendChild( new FileNode( finfo.fileName(), _state == SelSome ? SelNone : _state ) );
                    }
                }
            }
        }
...
void TapeDriveNode::expanding( bool expand )
{
    ...
    Q3PtrListIterator<Archive> f( TapeManager::instance()->getMountedTape()->getChildren() );
    for ( ; f.current(); ++f ) {
...
void TapeDriveNode::slotTapeModified( Tape* tape )
{
    ...
    // Add/update existing archives.
    Q3PtrListIterator<Archive> i( tape->getChildren() );
