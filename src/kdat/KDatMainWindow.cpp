        ...
        : KMainWindow(0), _destroyed( FALSE )
{
    ...
    _fileMenu->insertSeparator();
    _fileMenu->insertItem( SmallIcon("application-exit"), i18n( "&Quit" )                , this, SLOT( fileQuit() ), CTRL + Key_Q );
...
void KDatMainWindow::fileBackup()
{
    ...
    QString msg = i18n("KDat will dump your files properly to tape, but may not be able\n"
			...
			Options::instance()->getTapeDevice());
    ...
    if ( ( _tree->getCurrentItem() ) && ( ((Node*)_tree->getCurrentItem())->isType( Node::BackupProfileNodeType ) ) ) {
        ...
        QString name;
        name = i18n( "Archive created on %1", KGlobal::locale()->formatDate(QDate::currentDate(), KLocale::ShortDate) );
        name = name.trimmed();
...
void KDatMainWindow::fileIndexTape()
{
    ...
    if ( result == KMessageBox::Continue) {
        ...
        if ( dlg.exec() == QDialog::Accepted ) {
            QString title;
            title = i18n( "KDat: %1", TapeManager::instance()->getMountedTape()->getName() );
            setCaption( title );
            setIconText( title );
...
void KDatMainWindow::fileDeleteArchive()
{
    ...
    if ( i.current() ) {
        ...
        QString msg =
             i18n( "Really delete the archive '%1'?", archive->getName() );
        int result = KMessageBox::warningContinueCancel( this,
...
void KDatMainWindow::fileDeleteIndex()
{
    ...
    QString msg =
         i18n( "Really delete the index for '%1'?", tape->getName() );
    int result = KMessageBox::warningContinueCancel( this,
...
void KDatMainWindow::fileFormatTape()
{
    ...
    if (result == KMessageBox::Continue ) {
        QString name;
        name = i18n( "Tape created on %1", KGlobal::locale()->formatDate(QDate::currentDate(), KLocale::ShortDate) );
        FormatOptDlg dlg( name.trimmed(), this );
        if ( dlg.exec() != QDialog::Accepted ) {
            return;
        }
...
void KDatMainWindow::fileDeleteBackupProfile()
{
    ...
    QString msg =
         i18n("Really delete backup profile '%1'?", backupProfile->getName());
    int result = KMessageBox::warningContinueCancel( this,
            ...
            if ( tape ) {
                ...
                if ( TapeDrive::instance()->isReadOnly() ) {
                    ...
                    int result = KMessageBox::questionYesNo( this,
                          i18n( "This tape has not been formatted by KDat.\n\nWould you like to format it now?" ), QString(), i18n("Format"), i18n("Do Not Format"));
    ...
    for ( ; ii != filesTmp.end(); ++ii ) {
	    ...
	    if( stop_flag == TRUE ) break;
            ...
            msg = i18n("Estimating backup size: %1, %2",
             Util::kbytesToString( size / 2 ),
             KStringHandler::csqueeze(*path, 60));
            status( msg );
            KApplication::kApplication()->processEvents();
            dir.setPath( *path );
...
void KDatMainWindow::configureUI( Tape* tape )
{
    ...
    if ( tape ) {
        QString title;
        title = i18n( "KDat: %1", tape->getName() );
        setCaption( title );
        setIconText( title );
    } else {
        setCaption( i18n( "KDat: <no tape>" ) );
...
void KDatMainWindow::create_backup_dialog()
{
  ...
  _cancel = new KPushButton( KStandardGuiItem::cancel(), _backupdialog );
  ...
  _continue = new KPushButton( KStandardGuiItem::cont(), _backupdialog );
