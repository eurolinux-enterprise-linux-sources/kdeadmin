          ...
          _done( FALSE )
{
    ...
    Q3PtrListIterator<Range> i( _ranges.getRanges() );
    _archiveSize = 0;
    for ( ; i.current(); ++i ) {
...
void VerifyDlg::show()
{
    ...
    *_proc << Options::instance()->getTarCommand();
    ...
    connect( _proc, SIGNAL( wroteStdin( K3Process* ) ), this, SLOT( slotWroteStdin( K3Process* ) ) );

    _startTime = time( NULL );
    startTimer( 100 );

    _proc->start( K3Process::NotifyOnExit, K3Process::All );

    QDialog::show();
}

void VerifyDlg::slotProcessExited( K3Process* )
{
...
void VerifyDlg::slotStdout( K3Process*, char* buf, int len )
{
    QString data;
...
void VerifyDlg::slotWroteStdin( K3Process* )
{
    _wroteStdin = TRUE;
}
	...
	/* 2002-01-30 LEW */
        ...
        while ( bytesToRead > 0 ) {
	      ...
	      /* 2002-01-30 LEW */
            ...
            if ( count < 0 ) {
                kError() << i18n( "failed while reading tape data.\n" );
                _proc->closeStdin();
                delete [] buf;
                return;
            }
